...
def parse_custom_rec_video_metadata(item):
    '''
    Parses and processes raw output and returns video_id, channel_title, channel_id, video_publish_date, video_title, video_description, video_category, video_thumbnail, collection_date.
    ...
    video_meta = {
        ...
        "title": item["snippet"].get("title"),
        "published_date": parse_yt_datetime(item["snippet"].get("publishedAt")),
        "description": item["snippet"].get("description"),
        "category": item["snippet"].get("categoryId"),
        "link": f"https://www.youtube.com/watch?v={item['id'].get('videoId')}",
        "channel": {
            "name": item["snippet"].get("channelTitle"),
            "id": item["snippet"].get("channelId"),
        },
    }
