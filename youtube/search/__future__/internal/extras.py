...
from youtube.search.__future__.internal.constants import *
from youtube.search.__future__.internal.json import loads
    ...
    async def __getVideoComponent(self, element: dict, mode: str) -> dict:
        ...
        if mode in ['getInfo', None]:
            component = {
                ...
                'publishDate': await self.__getValue(element,
                                                     ['microformat', 'playerMicroformatRenderer', 'publishDate']),
                'uploadDate': await self.__getValue(element,
...
class PlaylistInternal:
    ...
    async def __getNextComponents(self) -> None:
        ...
        continuationElements = await self.__getValue(self.responseSource,
        ...
        for videoElement in continuationElements:
            if playlistVideoKey in videoElement.keys():
                videoComponent = {
                    ...
                    'channel': {
                        'name': await self.__getValue(videoElement,
                                                      [playlistVideoKey, 'shortBylineText', 'runs', 0, 'text']),
                        'id': await self.__getValue(videoElement, [playlistVideoKey, 'shortBylineText', 'runs', 0,
                                                                   'navigationEndpoint', 'browseEndpoint', 'browseId']),
                    },
                    'duration': await self.__getValue(videoElement, [playlistVideoKey, 'lengthText', 'simpleText']),
                    'accessibility': {
                        'title': await self.__getValue(videoElement,
                                                       [playlistVideoKey, 'title', 'accessibility', 'accessibilityData',
                                                        'label']),
                        'duration': await self.__getValue(videoElement,
                                                          [playlistVideoKey, 'lengthText', 'accessibility',
        ...
        if mode in ['getInfo', None]:
            for infoElement in element['info']:
                if playlistPrimaryInfoKey in infoElement.keys():
                    component = {
                        'id': await self.__getValue(infoElement,
                                                    [playlistPrimaryInfoKey, 'title', 'runs', 0, 'navigationEndpoint',
                                                     'watchEndpoint', 'playlistId']),
                        'title': await self.__getValue(infoElement,
                                                       [playlistPrimaryInfoKey, 'title', 'runs', 0, 'text']),
                        'videoCount': await self.__getValue(infoElement,
                                                            [playlistPrimaryInfoKey, 'stats', 0, 'runs', 0, 'text']),
                        'viewCount': await self.__getValue(infoElement,
                                                           [playlistPrimaryInfoKey, 'stats', 1, 'simpleText']),
                        'thumbnails': await self.__getValue(infoElement, [playlistPrimaryInfoKey, 'thumbnailRenderer',
                ...
                if playlistSecondaryInfoKey in infoElement.keys():
                    component = {
                        'channel': {
                            'name': await self.__getValue(infoElement,
                            ...
                            'id': await self.__getValue(infoElement,
                                                        [playlistSecondaryInfoKey, 'videoOwner', 'videoOwnerRenderer',
                                                         ...
                                                         'browseId']),
                            'thumbnails': await self.__getValue(infoElement, [playlistSecondaryInfoKey, 'videoOwner',
                                                                              'videoOwnerRenderer', 'thumbnail',
                                                                              'thumbnails']),
                        },
                    }
                    component['channel']['link'] = 'https://www.youtube.com/channel/' + component['channel']['id']
            ...
            for videoElement in element['videos']:
                if playlistVideoKey in videoElement:
                    videoComponent = {
                        'id': await self.__getValue(videoElement, [playlistVideoKey, 'videoId']),
                        'title': await self.__getValue(videoElement, [playlistVideoKey, 'title', 'runs', 0, 'text']),
                        'thumbnails': await self.__getValue(videoElement,
                                                            [playlistVideoKey, 'thumbnail', 'thumbnails']),
                        'channel': {
                            'name': await self.__getValue(videoElement,
                                                          [playlistVideoKey, 'shortBylineText', 'runs', 0, 'text']),
                            'id': await self.__getValue(videoElement, [playlistVideoKey, 'shortBylineText', 'runs', 0,
                        ...
                        'duration': await self.__getValue(videoElement, [playlistVideoKey, 'lengthText', 'simpleText']),
                        'accessibility': {
                            'title': await self.__getValue(videoElement, [playlistVideoKey, 'title', 'accessibility',
                                                                          'accessibilityData', 'label']),
                            'duration': await self.__getValue(videoElement,
                                                              [playlistVideoKey, 'lengthText', 'accessibility',
                                                               'accessibilityData', 'label']),
